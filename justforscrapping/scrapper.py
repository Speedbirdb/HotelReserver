...
"""def extract_hotel_info(url):
    ...
    hotel_info = []
    for el in soup.find_all("div", {"data-testid": "property-card"})[:10]:  # Extracting data for the first 10 hotels
        title = el.find("div", {"data-testid": "title"})
        address = el.find("span", {"data-testid": "address"})
        distance = el.find("span", {"data-testid": "distance-to-city-center"})
        rating = el.find("div", {"data-testid": "review-score"})
        price = el.find("span", {"data-testid": "price-and-discounted-price"})
        
        if title and address and distance and rating and price:  # Check if all elements exist
            hotel_data = {
                "title": title.text.strip(),
                "address": address.text.strip(),
                "distance_to_city_center": distance.text.strip() if distance else "N/A",
